...
train_set=alex_train-clean-460_w2vU2.0_v1
valid_set=alex_dev-clean_w2vU2.0_v1
test_sets="alex_test-clean_nopunc"

train_config=conf/tuning/sls_8gpu_train_gst+xvector_transformer-guided_attn_v1.yaml
...
./tts-w2vU2.0_v1.sh \
    ...
    --tag sls_train_gst+xvector_transformer-guided_attn_v1_raw_phn_none-w2vU2.0_v1 \
    --tts_stats_dir exp/tts_stats_raw_phn_none-w2vU2.0_v1 \
    --inference_model ${inference_model}.pth \
    --inference_tag decode_transformer_${inference_model}_vctk_hifigan \
    --vocoder_file $vctk_hifigan \
    ${opts} "$@"
done

    #--inference_tag decode_transformer_${inference_model}_melgan \
    #--vocoder_file $melgan \
